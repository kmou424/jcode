...
impl App {
    ...
    fn open_model_picker_inner(&mut self, preserve_input: bool) {
        ...
        // In SSH mode config() is the remote override (remote machine's
        // config.toml); locally it is the local file. Either way it is the
        // authoritative provider/model config for this session.
    ...
    /// Apply a `list_sessions` sideband reply arriving from the remote bridge (SSH mode):
    ...
    pub(super) fn apply_remote_session_list(
        &mut self,
        entries: Vec<jcode_app_core::ssh_ops::SshSessionEntry>,
    ) -> bool {
    ...
    /// Apply a `session_preview` sideband reply arriving from the remote bridge to
    /// the open remote session picker. Returns true when a redraw is needed.
    pub(super) fn apply_remote_session_preview(
        &mut self,
        session_id: &str,
        messages: Vec<jcode_app_core::ssh_ops::SshPreviewMessage>,
    ) -> bool {
        let Some(picker_cell) = self.session_picker_overlay.as_ref() else {
            return false;
        };
