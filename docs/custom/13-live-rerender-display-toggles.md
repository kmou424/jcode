# 13: Re-render transcript history live on display toggle changes

Patch: `[13] feat(tui): re-render history live on display toggle changes`
...
Follow-up change on the same branch (`[13] feat(tui): mode-agnostic
...
- `compact` keeps its live `✻ thinking…` header while streaming, but the
  ...
  `duration_secs` (identical semantics to patch 12: ceil to one decimal,
